(() => {
  ...
  document.addEventListener('click', event => {
    ...
    if (button?.classList.contains('code-preview__button--codepen')) {
      ...
      if (!isReact) {
        htmlTemplate =
          `<script type="module" src="https://cdn.jsdelivr.net/npm/pure-uikit@${pureUIVersion}/${cdndir}/pure-ui.js"></script>\n` +
          `\n${htmlExample}`;
        jsTemplate = '';
      }
      ...
      if (isReact) {
        ...
        jsTemplate =
          ...
          `import { setBasePath } from 'https://esm.sh/pure-uikit@${pureUIVersion}/${cdndir}/utilities/base-path';\n` +
          `\n` +
          `// Set the base path for Pure UI assets\n` +
          `setBasePath('https://esm.sh/pure-uikit@${pureUIVersion}/${npmdir}/')\n` +
      ...
      // CSS templates
      cssTemplate =
        `@import 'https://cdn.jsdelivr.net/npm/pure-uikit@${pureUIVersion}/${cdndir}/themes/${
          isDark ? 'dark' : 'light'
        }.css';\n` +
        '\n' +
        'body {\n' +
